...
- (void)setup
{
    ...
    //self.floatingLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height/5)];
    self.floatingLabel = [[UILabel alloc]init];
    self.floatingLabel.frame = (CGRect) {0,0,self.frame.size.width,[self.dataSource lineHeightForTokenInField:self]};
    [self addSubview:self.floatingLabel];
    [self.floatingLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.mas_top).with.offset(5); //with is an optional semantic filler
        make.left.equalTo(self.mas_left).with.offset(0);
        make.right.equalTo(self.mas_right).with.offset(0);
    }];
    self.floatingLabel.textColor = UIColor.lightGrayColor;
    [self.floatingLabel setText:@""];
...
- (CGSize)intrinsicContentSize
{
    ...
    return CGSizeMake(totalRect.size.width, totalRect.size.height+30) ;
}
...
- (void)reloadData
{
    ...
    self.textField.frame = (CGRect) {0,50,50,[self.dataSource lineHeightForTokenInField:self]};
